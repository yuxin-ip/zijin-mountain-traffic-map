...
(async () => {
  ...
  try {
    ...
    assert.equal(beforeColor, '#5fc8ff', 'Cycling permitted / motor traffic restricted must be blue');
    assert(await page.locator('.traffic-road[stroke="#35e5be"]').count() > 0, 'Unrestricted reference roads must be green');
    ...
    assert.equal(await page.locator('[data-road-key="143346138"] .traffic-road').getAttribute('stroke'), '#35e5be');
    for (const vehicle of ['机动车 · 无预约', '机动车 · 已预约', '普通自行车', '电动自行车']) {
      await page.getByRole('button', { name: vehicle, exact: true }).click();
      assert.equal(await page.locator('.traffic-road:not([stroke="#35e5be"])').count(), 0, 'No explicit off-peak restriction in current dataset: show green');
    }
    await page.getByRole('button', { name: '陵园路 · 步行道 未作特别限制', exact: true }).click();
    assert((await page.locator('main').innerText()).includes('未明确规定管控时段外的专项限制'));
    ...
    await page.locator('.map-drag-cache[data-ready="true"]').waitFor({ state: 'attached' });
    await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2);
    await page.mouse.down();
    await page.mouse.move(box.x + box.width / 2 + 90, box.y + box.height / 2 + 30, { steps: 5 });
    await page.waitForFunction(() => document.querySelector('[data-testid="traffic-map"] > svg').style.transform.includes('translate3d'));
    assert.equal(await page.locator('.map-drag-cache').evaluate(node => getComputedStyle(node).visibility), 'visible', 'Dragging uses the cached map surface');
    await page.screenshot({ path: 'artifacts/map-drag.png', fullPage: true });
    await page.mouse.up();
    assert.notEqual((await state()).view, beforeDrag, 'Dragging must pan');
    assert.equal(await svg.evaluate(node => node.style.transform), '', 'Finishing a drag must restore crisp geometry');
    assert.equal(await svg.evaluate(node => getComputedStyle(node).opacity), '1');
    assert.equal(await page.locator('.map-drag-cache').evaluate(node => getComputedStyle(node).visibility), 'hidden');
    // Exercise refilling after a drag longer than the cached edge margin.
    await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2);
    await page.mouse.down();
    await page.mouse.move(box.x + box.width / 2 + 310, box.y + box.height / 2, { steps: 25 });
    await page.mouse.up();
    assert.equal(await svg.evaluate(node => node.style.transform), '');
