...
(async () => {
  ...
  const samples = [];
  for (let run = 0; run < 3; run++) samples.push(await page.evaluate(async amplitude => {
    ...
    const capture = svg.setPointerCapture, rootCapture = root.setPointerCapture;
    svg.setPointerCapture = root.setPointerCapture = () => {};
    ...
    for (let i = 0; i < 90; i++) {
      ...
      for (let j = 0; j < 3; j++) send('pointermove', Math.sin((i * 3 + j) / 270 * Math.PI * 2) * amplitude);
    }
    send('pointerup', 0);
  ...
  }, Number(process.env.DRAG_DISTANCE || 450)));
